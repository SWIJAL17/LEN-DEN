                ...
                <tbody>
                  {repayments.map((r) => (
                    <tr key={r.installment_no} className="border-b border-white/5 hover:bg-white/[0.02]">
                      ...
                      <td className="py-3 px-2 text-right text-cyan-400">₹{r.interest}</td>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="glass-card p-6">
              ...
              {investAmount && parseFloat(investAmount) > 0 && (
                <div className="bg-dark-800/50 rounded-xl p-4 mb-4 space-y-2 text-sm">
                  <div className="flex justify-between"><span className="text-white/40">Your investment</span><span className="text-white">₹{parseFloat(investAmount).toLocaleString('en-IN')}</span></div>
                  <div className="flex justify-between"><span className="text-white/40">Expected return</span><span className="text-emerald-400">₹{(parseFloat(investAmount) * 1.12).toFixed(0).replace(/\B(?=(\d{3})+(?!\d))/g, ',')}</span></div>
                  <div className="flex justify-between"><span className="text-white/40">Estimated profit</span><span className="text-emerald-400">₹{(parseFloat(investAmount) * 0.12).toFixed(0).replace(/\B(?=(\d{3})+(?!\d))/g, ',')}</span></div>
                </div>
              )}
            ...
            <div className="space-y-3">
              {contributors.map((c, i) => (
                <div key={i} className="flex items-center gap-3 p-3 rounded-xl bg-dark-800/40">
                  ...
                  <span className="text-xs text-emerald-400 font-semibold">+₹{(c.expected_return - c.amount).toLocaleString('en-IN')}</span>
                </div>
              ))}
            </div>
          </motion.div>
